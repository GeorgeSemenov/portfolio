...
$(document).ready(function(){
  ...
  for(i = 0; i< elements.length; i++){
    ...
    $(element).on( "mouseenter", ()=>{if($(window).width() > 1199){chachgeBottomValue(element,showedBottom)}})
    $(element).on( "mouseleave", ()=>{if($(window).width() > 1199){chachgeBottomValue(element,defaultBottom)}})
    $(window).on('resize', ()=>{chachgeBottomValue(element,mobileBottom)})
  }
...
function chachgeBottomValue(element,bottomValue){
	$(element).children(".portfolio-screen__example-git-href").css({"bottom":bottomValue})
	$(element).children(".portfolio-screen__example-description-button").css({"bottom":bottomValue})
}
