	...
	{//Первый объект
		...
		plugins:[
			...
			new HtmlWebpackPlugin({// создаём страничку html, при этом она подключает все файлы, указанные в свойствах объекта output (используется для npm run build)
				...
				scriptLoading: 'defer',//При закгрузке скрипта ему будет выставлен парамметр defer, примечательно, если я изменю значение на async, параметр всё равно будет defer, почему так, я не знаю.
			...
			// 	patterns: 
			// 	[
	  //       { from: path.join(__dirname, 'source/img/favicon/manifest.json'), 
